...
fun main(args: Array<String>) {
    ...
    requestAttributes.put(AttributeKey("Authorization"), "Basic $password")
    //println(readGitCommittedLines("tos", start, end, requestAttributes))
    println(readJiraContentModifications("tos", start, end, requestAttributes))
}
...
fun readGitCommittedLines(project: String, fromTime: Long, toTime: Long, requestAttributes: Attributes): Map<String, Int> {
    ...
    val statistics = get("$FISH_EYE_URL/search-v1/queryAsRows/$project?query=$query", requestAttributes)
...
fun readConfluenceContentModifications(space: String, fromTime: Long, toTime: Long,
                                       requestAttributes: Attributes): Map<String, Int> {
    val from = formatTime(fromTime)
    val to = formatTime(toTime)
    val query = URLEncoder.encode("space=$space and (lastmodified >= $from and lastmodified <= $to " +
            "or created >= $from and created <= $to", "utf-8")
    val statistics = get("$CONFLUENCE_URL/search?cql=$query&expand=version", requestAttributes)
...
fun readJiraContentModifications(project: String, fromTime: Long, toTime: Long,
    ...
    val statistics = get("$JIRA_URL/search?jpl=$query&fields=changelog&expand=changelog", requestAttributes)
...
fun get(url: String, requestAttributes: Attributes): JsonObject {
